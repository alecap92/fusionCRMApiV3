...
exports.getMarketingDashboard = void 0;
const googleAnalyticsService_1 = require("../../services/analytics/googleAnalyticsService");
const credentials = {
    ...
    client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/google-analytics%40fusioncrm-project.iam.gserviceaccount.com",
...
const getMarketingDashboard = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const userMetricsReport = yield (0, googleAnalyticsService_1.runAnalyticsReport)(credentials, {
        ...
        // Fuentes de tráfico
        const trafficSourcesReport = yield (0, googleAnalyticsService_1.runAnalyticsReport)(credentials, {
            propertyId: "257042282",
        ...
        const eventsReport = yield (0, googleAnalyticsService_1.runAnalyticsReport)(credentials, {
            propertyId: "257042282",
            dateRanges: [{ startDate: currentMonthStart, endDate: currentMonthEnd }],
        ...
        const topPagesReport = yield (0, googleAnalyticsService_1.runAnalyticsReport)(credentials, {
            propertyId: "257042282",
            dateRanges: [{ startDate: currentMonthStart, endDate: currentMonthEnd }],
        ...
        const rowsActual = userMetricsReport.rows.filter((row) => row.dimensionValues.length > 0 &&
            row.dateRangeIndices &&
            row.dateRangeIndices[0] === 0);
        const rowsAnterior = userMetricsReport.rows.filter((row) => row.dimensionValues.length > 0 &&
            row.dateRangeIndices &&
        ...
        if (rowsActual.length === 0 || rowsAnterior.length === 0) {
            ...
            const rowsFilteredByDate = userMetricsReport.rows.filter((row) => {
            ...
            const rowsFilteredByDateAnterior = userMetricsReport.rows.filter((row) => {
                const rowDate = new Date(formatDate(row.dimensionValues[0].value));
        ...
        const totalActiveUsersActual = rowsActual.reduce((sum, row) => sum + parseInt(row.metricValues[0].value), 0);
        const totalNewUsersActual = rowsActual.reduce((sum, row) => sum + parseInt(row.metricValues[1].value), 0);
        const avgBounceRateActual = rowsActual.reduce((sum, row) => sum + parseFloat(row.metricValues[2].value), 0) / (rowsActual.length || 1);
        const avgSessionDurationActual = rowsActual.reduce((sum, row) => sum + parseFloat(row.metricValues[3].value), 0) / (rowsActual.length || 1);
        // Totales periodo anterior
        const totalActiveUsersAnterior = rowsAnterior.reduce((sum, row) => sum + parseInt(row.metricValues[0].value), 0);
        const totalNewUsersAnterior = rowsAnterior.reduce((sum, row) => sum + parseInt(row.metricValues[1].value), 0);
        const avgBounceRateAnterior = rowsAnterior.reduce((sum, row) => sum + parseFloat(row.metricValues[2].value), 0) / (rowsAnterior.length || 1);
        const avgSessionDurationAnterior = rowsAnterior.reduce((sum, row) => sum + parseFloat(row.metricValues[3].value), 0) / (rowsAnterior.length || 1);
        ...
        const variacionActiveUsers = calcVar(totalActiveUsersActual, totalActiveUsersAnterior);
        const variacionNewUsers = calcVar(totalNewUsersActual, totalNewUsersAnterior);
        const variacionBounceRate = calcVar(avgBounceRateActual, avgBounceRateAnterior);
        const variacionSessionDuration = calcVar(avgSessionDurationActual, avgSessionDurationAnterior);
        // Procesar métricas de usuarios (solo periodo actual para daily)
        const processedMetrics = rowsActual
            .filter((row) => row.metricValues.some((metric) => metric.value !== "0"))
            .map((row) => ({
            date: formatDate(row.dimensionValues[0].value),
            activeUsers: parseInt(row.metricValues[0].value),
            newUsers: parseInt(row.metricValues[1].value),
            bounceRate: roundNumber(parseFloat(row.metricValues[2].value)),
            avgSessionDuration: secondsToMinutes(parseFloat(row.metricValues[3].value)),
        }));
        // Procesar fuentes de tráfico
        const processedTrafficSources = trafficSourcesReport.rows
            .filter((row) => parseInt(row.metricValues[0].value) > 0)
            .map((row) => ({
        ...
        // Procesar eventos
        const processedEvents = eventsReport.rows
            .filter((row) => parseInt(row.metricValues[0].value) > 0)
            .map((row) => ({
        ...
        const processedPages = topPagesReport.rows
            .filter((row) => parseInt(row.metricValues[0].value) > 0)
            .map((row) => ({
...
exports.getMarketingDashboard = getMarketingDashboard;
