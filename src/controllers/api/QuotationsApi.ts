...
import { generateQuotationPdf } from "../../services/quotation/printQuotationService";
    ...
    */
    ...
    try {
      ...
      if (contactExists) {
        ...
        const needsUpdate =
          (firstNameProp && firstNameProp.value !== clientName) ||
        ...
        if (needsUpdate) {
          ...
          if (mobileProp && mobileProp.value !== clientMobile) {
            await ContactModel.updateOne(
              ...
              { $set: { "properties.$.value": clientMobile } }
            );
          }
...
export const downloadQuotationApi = async (
  req: IApiAuthRequest,
  res: Response
) => {
  try {
