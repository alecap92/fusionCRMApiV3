...
export const createDocument = async (req: Request, res: Response) => {
  try {
    ...
    const { name, description, tags, metadata, organizationId, uploadedBy } =
      req.body;
    ...
    if (!uploadedBy || !organizationId) {
      return res.status(400).json({
        message:
          "No se ha proporcionado el usuario que sube el documento o la organización",
      });
    }
    ...
    const uploadedFile = await subirArchivo(
      req.file.buffer,
...
export const getAllDocuments = async (req: Request, res: Response) => {
  try {
    ...
    if (
      organizationId &&
      mongoose.Types.ObjectId.isValid(organizationId as string)
    ) {
      filter.organizationId = organizationId;
    } else if (req.user?.organizationId) {
      // Si no se proporciona, usar la organización del usuario
      filter.organizationId = new mongoose.Types.ObjectId(
...
export const getDocumentsByOrganization = async (
  req: Request,
  res: Response
) => {
  try {
