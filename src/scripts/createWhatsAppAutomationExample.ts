...
async function createWhatsAppAutomationExample() {
  try {
    ...
    console.log(
      `📊 Creando automatización para organización: ${organization.companyName}`
    );
    console.log(`👤 Usuario creador: ${user.firstName} ${user.lastName}`);

    // Crear una automatización de bienvenida con el editor visual
    const welcomeAutomation = await AutomationModel.create({
      ...
      nodes: [
        ...
        },
        {
          ...
          data: {
            message:
              "Entiendo que necesitas ayuda con otra opción. Un agente se pondrá en contacto contigo pronto. 👨‍💼",
          },
          next: ["action_4"],
    ...
    console.log(
      "✅ Automatización de bienvenida creada:",
      welcomeAutomation.name
    );

    // Crear una automatización por palabras clave
    const keywordAutomation = await AutomationModel.create({
    ...
    console.log(
      "✅ Automatización de palabras clave creada:",
      keywordAutomation.name
    );
