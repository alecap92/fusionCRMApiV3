...
const createSampleAutomation = async () => {
  try {
    // Conectar a MongoDB
    ...
    const chatbotAutomation = {
      ...
      description:
        "Automatización que responde preguntas frecuentes y deriva a un agente si es necesario",
      ...
      nodes: [
        ...
        // 16. Finalizar
        {
          ...
          data: {
            message:
              "¡Gracias por contactarnos! Si necesitas más ayuda, no dudes en escribirnos nuevamente. ¡Que tengas un excelente día! 🌟",
          },
        },
      ],
    };
    ...
    const automation = await AutomationModel.create(chatbotAutomation);
    console.log(
      `✅ Automatización creada: ${automation.name} (ID: ${automation._id})`
    );
    ...
    const keywordAutomation = {
      ...
      description:
        "Responde automáticamente a preguntas frecuentes basadas en palabras clave",
      organizationId: organization._id,
      isActive: true,
      triggerType: "keyword",
      triggerConditions: {
    ...
    const automation2 = await AutomationModel.create(keywordAutomation);
    console.log(
      `✅ Automatización creada: ${automation2.name} (ID: ${automation2._id})`
    );

    console.log("\n🎉 ¡Automatizaciones de ejemplo creadas exitosamente!");
    console.log("\n📝 Resumen:");
    console.log(
      `1. ${chatbotAutomation.name}: Flujo completo con menú y condicionales`
    );
    console.log(
      `2. ${keywordAutomation.name}: Respuesta automática a palabras clave`
    );
  } catch (error) {
...
export default createSampleAutomation;
